/****************************************************************************
 * arch/arm/src/stm32n6/stm32_iwdg.c
 *
...
#if defined(CONFIG_WATCHDOG) && defined(CONFIG_STM32N6_IWDG)
 ...
 *
 *  Fmin = Flsi / 1024    (STM32N6 supports up to /1024 prescaler)
 *
 * So the maximum delay (in milliseconds) is then:
 *
 *   1000 * IWDG_RLR_MAX / Fmin
 *
 * For example, if Flsi = 32Khz (the nominal value), then:
 *
 *   Fmin = 31.25
 *   1000 * 4095 / Fmin = 131,040 MSec
 */
...
#ifndef CONFIG_STM32N6_IWDG_DEFTIMOUT
#  define CONFIG_STM32N6_IWDG_DEFTIMOUT IWDG_MAXTIMEOUT
#endif

#ifndef CONFIG_DEBUG_WATCHDOG_INFO
#  undef CONFIG_STM32N6_IWDG_REGDEBUG
#endif

/* It appears that the prescaler and reload registers can only be set up
 * once.  After that, the SR register's PVU and RVU bits never go to zero.
 * So we defer setting up these registers until the watchdog is started,
 * then refuse any further attempts to change timeout.
 */

#define CONFIG_STM32N6_IWDG_ONETIMESETUP 1

/* STM32N6 max prescaler index is 8 (/1024), vs H7's max of 6 (/256) */

#define IWDG_PR_MAX     8
...
#ifdef CONFIG_STM32N6_IWDG_REGDEBUG
...
#ifndef CONFIG_STM32N6_IWDG_ONETIMESETUP
  while ((stm32_getreg(STM32_IWDG_SR) & (IWDG_SR_PVU | IWDG_SR_RVU)) != 0);
#endif

  /* Set the prescaler */

  stm32_putreg((uint32_t)priv->prescaler << IWDG_PR_SHIFT, STM32_IWDG_PR);

  /* Set the reload value */

  stm32_putreg((uint32_t)priv->reload, STM32_IWDG_RLR);
...
#ifdef CONFIG_STM32N6_IWDG_ONETIMESETUP
...
static int stm32_stop(struct watchdog_lowerhalf_s *lower)
{
  /* There is no way to disable the IWDG timer once it has been started */
                            ...
                            uint32_t timeout)
{
  ...
  /* Once started, the prescaler and reload registers cannot be changed */
  ...
  for (prescaler = 0; ; prescaler++)
    {
      ...
      /* Get the IWDG counter frequency in Hz. For a nominal 32Khz LSI
       * clock, this is value in the range of 8000 and 31.25.
       */
      ...
      /* If this reload value is less than the maximum or we are at the
       * maximum prescaler value, then break out of the loop to use these
       * settings.
       */
...
#endif /* CONFIG_WATCHDOG && CONFIG_STM32N6_IWDG */
