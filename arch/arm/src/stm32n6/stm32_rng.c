/****************************************************************************
 * arch/arm/src/stm32n6/stm32_rng.c
 *
...
static int stm32_rng_initialize(void)
{
  ...
  /* Enable RNG clock via AHB3 SET register */

  putreg32(RCC_AHB3ENR_RNGEN, STM32_RCC_AHB3ENSR);

  /* Enable RNG clock during CSLEEP (WFI) so data generation continues */

  putreg32(RCC_AHB3LPENR_RNGLPEN, STM32_RCC_AHB3LPENSR);

  /* STM32N6 requires conditioning reset before RNG can generate data.
   * Sequence: disable RNG, set CONDRST=1, clear CONDRST=0, wait for
   * hardware to clear CONDRST, then enable RNG.
   */
  ...
  /* Set CONDRST=1 (with CED=0 to enable clock error detection) */
  ...
  /* Clear CONDRST=0 to start conditioning reset */

  regval &= ~RNG_CR_CONDRST;
  putreg32(regval, STM32_RNG_CR);

  /* Wait for conditioning reset to complete (CONDRST bit reads 0) */

  for (timeout = 0; timeout < 100000; timeout++)
    {
      if ((getreg32(STM32_RNG_CR) & RNG_CR_CONDRST) == 0)
        {
          break;
        }
    }
