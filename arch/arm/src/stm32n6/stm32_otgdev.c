...
#if defined(CONFIG_USBDEV) && (defined(CONFIG_STM32N6_USB1_OTG_HS) || \
...
#if defined(CONFIG_STM32N6_USB1_USBDEV)
...
#  define STM32_OTG_FIFO_SIZE   3808  /* GHWCFG3 DFIFODepth=952 words */
#elif defined(CONFIG_STM32N6_USB2_USBDEV)
...
#  define STM32_OTG_FIFO_SIZE   3808  /* GHWCFG3 DFIFODepth=952 words */
#else
#  error "No USB OTG device role configured"
#endif

#if defined(CONFIG_STM32N6_USB1_USBDEV) && defined(CONFIG_STM32N6_USB2_USBDEV)
...
#if defined(CONFIG_STM32N6_USBDEV_REGDEBUG) && defined(CONFIG_DEBUG_FEATURES)
...
/* stm32_txfifo_write always available — needed as DMA fallback */
...
#ifdef CONFIG_STM32N6_OTG_DMA
  /* Descriptor DMA: build IN descriptor and point DIEPDMA to it.
   * EP0 uses a bounce buffer; other EPs use the request buffer directly.
   */
...
static inline void stm32_enuminterrupt(struct stm32_usbdev_s *priv)
{
  ...
  /* Set USB turn-around time based on actual enumerated speed.
   * Read DSTS.ENUMSPD: 0=HS, 1=FS(48MHz PHY), 3=FS(6MHz PHY)
   */

  {
    uint32_t dsts = stm32_getreg(STM32_OTG_DSTS);
    uint32_t enumspd = (dsts >> 1) & 3;
    uint32_t trdt;
    ...
    regval  = stm32_getreg(STM32_OTG_GUSBCFG);
    regval &= ~OTG_GUSBCFG_TRDT_MASK;
    regval |=  OTG_GUSBCFG_TRDT(trdt);
    stm32_putreg(regval, STM32_OTG_GUSBCFG);
  }

  /* Re-configure EP0 and re-arm for SETUP reception.
   * DWC2 v5.00 may require re-programming DOEPTSIZ0 after ENUMDNE.
   */
...
static void stm32_hwinitialize(struct stm32_usbdev_s *priv)
{
  ...
  /* N6 uses internal HS UTMI+ PHY.
   * Per ST HAL USB_CoreInit(): clear TSDPS, select data line pulsing
   * via utmi_txvalid.  No PHYSEL or ULPI bits needed.
   */

  /* Per ST HAL USB_CoreInit for embedded HS PHY: only clear TSDPS.
   * Do NOT touch PHYSEL or ULPI bits — core reset will set defaults.
   */

  regval = stm32_getreg(STM32_OTG_GUSBCFG);
  regval &= ~OTG_GUSBCFG_TSPDS;
  stm32_putreg(regval, STM32_OTG_GUSBCFG);
  ...
  /* After core reset, set turnaround time.
   * TRDT = 9 for HS, adjusted later in enum interrupt based on speed.
   */
  ...
  /* Configure GCCFG for STM32N6 (register layout differs from STM32H7!):
   * - Disable pulldown resistors (device mode, not host)
   * - Override VBUS valid in GCCFG (not GOTGCTL like on H7)
   */
...
#ifndef CONFIG_STM32N6_OTG_VBUS_SENSING
  /* Force VBUS valid via GCCFG override (per ST HAL USB_DevInit) */
   ...
   * Stale values can confuse the DWC2 v5.00 FIFO controller.
   */
...
void arm_usbinitialize(void)
{
  ...
  /* Small delay for HSE + divider to stabilize */

  up_mdelay(2);

  /* Enable VDD33USB supply for the USB HS PHY (per Zephyr N6 sequence):
   * 1. Enable USB33 voltage monitoring (SVMCR3 bit 2 = USB33VMEN)
   * 2. Wait for USB33RDY (SVMCR3 bit 18)
   * 3. Enable VddUSB (SVMCR3 bit 10 = USB33SV)
   *
   * PWR SVMCR3 already has VddIO2SV(bit8) + VddIO3SV(bit9) from boot.
   * We OR in the USB bits.
   */

#define PWR_SVMCR3       (STM32_PWR_BASE + 0x03c)
#define PWR_USB33VMEN    (1 << 2)
#define PWR_USB33SV      (1 << 10)
#define PWR_USB33RDY     (1 << 18)
#define PWR_VDDIO2SV     (1 << 8)
#define PWR_VDDIO3SV     (1 << 9)

  /* Step 1: Enable monitoring */

  putreg32(getreg32(PWR_SVMCR3) | PWR_USB33VMEN, PWR_SVMCR3);

  /* Step 2: Wait for USB33RDY */

  {
    int timeout = 10000;
    while ((getreg32(PWR_SVMCR3) & PWR_USB33RDY) == 0 && timeout-- > 0)
      {
        up_udelay(1);
      }
  }

  /* Step 3: Enable VddUSB supply */

  putreg32(getreg32(PWR_SVMCR3) | PWR_USB33SV, PWR_SVMCR3);
  up_mdelay(1);

  /* Enable UCPD1 for CC detection (USB Type-C sink mode).
   * Without UCPD, the TCPP0203 in dead-battery mode presents Rd on CC
   * but may not properly configure the USB 2.0 data path orientation.
   * UCPD1 base (Secure) = APB1 + 0xFC00 = 0x5000FC00
   * Need APB1 clock for UCPD1: APB1ENR2 bit 0 = UCPDEN
   */
...
#define STM32_UCPD1_BASE    0x5000FC00
  ...
  {
    uint32_t ucpd_cr;
    ucpd_cr = (1 << 9);    /* ANAMODE = 1 (SNK) */
    ucpd_cr |= (3 << 10);  /* CCENABLE = both CC1+CC2 */
   ...
   * OTGPHY1SEL = 0 (HSE/2 kernel clock)
   * OTGPHY1CKREFSEL = 1 (HSE/2 direct from oscillator as reference)
   * Note: CKREFSEL=0 doesn't work (PHY doesn't pull D+ high).
   */

#ifdef CONFIG_STM32N6_USB1_USBDEV
  modifyreg32(STM32_RCC_CCIPR6,
              RCC_CCIPR6_OTGPHY1SEL_MASK,
              RCC_CCIPR6_OTGPHY1SEL_HSE_DIV2);
  modifyreg32(STM32_RCC_CCIPR6, 0, RCC_CCIPR6_OTGPHY1CKREFSEL);
#else
  modifyreg32(STM32_RCC_CCIPR6,
              RCC_CCIPR6_OTGPHY2SEL_MASK, 0);
...
#ifdef CONFIG_STM32N6_OTG_DMA
  /* DMA mode: enable AXI interconnect clocks so OTG DMA (on AHB5)
   * can access AXISRAM via the AHB5→AXI bridge.
   */
  ...
  /* Disconnect device -- set soft disconnect */
...
void arm_usbuninitialize(void)
{
  ...
  /* Only access OTG registers if clocks are enabled */

  if (getreg32(STM32_RCC_AHB5ENR) &
      (RCC_AHB5ENR_OTG1EN | RCC_AHB5ENR_OTG2EN))
    {
  ...
  /* Disable USB clocks via AHB5ENCR (atomic clear) */
...
int usbdev_register(struct usbdevclass_driver_s *driver)
{
  ...
  if (ret)
  ...
  else
    {
      ...
      /* Speed will be set properly in stm32_enuminterrupt */
...
#endif /* CONFIG_USBDEV && (CONFIG_STM32N6_USB1_OTG_HS || CONFIG_STM32N6_USB2_OTG_HS) */
