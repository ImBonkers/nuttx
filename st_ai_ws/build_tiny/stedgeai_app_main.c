...
STAI_NETWORK_CONTEXT_DECLARE(m_tiny, STAI_TINY_CONTEXT_SIZE)

/* Array to store the data of the activation buffers */
STAI_ALIGNED(STAI_TINY_ACTIVATION_1_ALIGNMENT)
static uint8_t activations_1[STAI_TINY_ACTIVATION_1_SIZE_BYTES];
...
static stai_ptr m_inputs[STAI_TINY_IN_NUM];
static stai_ptr m_outputs[STAI_TINY_OUT_NUM];
static stai_ptr m_acts[STAI_TINY_ACTIVATIONS_NUM];
...
int aiInit(void)
{
  ...
  stai_tiny_set_activations(m_tiny, m_acts, STAI_TINY_ACTIVATIONS_NUM);
  stai_tiny_get_activations(m_tiny, m_acts, &_dummy);
  ...
  stai_tiny_get_inputs(m_tiny, m_inputs, &_dummy);

  /* Output buffers are allocated in the activations buffer */
  stai_tiny_get_outputs(m_tiny, m_outputs, &_dummy);
...
stai_return_code aiRun()
{
  ...
  res = stai_tiny_run(m_tiny, STAI_MODE_SYNC);
